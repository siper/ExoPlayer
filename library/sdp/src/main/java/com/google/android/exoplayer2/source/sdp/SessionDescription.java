...
public final class SessionDescription {
    ...
    public Information getInformation() { return info; }
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (matcher.find()) {
                    switch (matcher.group(1).charAt(0)) {
                        case 'v':
                            sesionBuilder.setVersion(ProtoVersion.parse(matcher.group(2).trim()));
                            break;

                        case 'o':
                            sesionBuilder.setOrigin(Origin.parse(matcher.group(2).trim()));
                            break;

                        case 's':
                            sesionBuilder.setSessionName(SessionName.parse(matcher.group(2).trim()));
                            break;
                        ...
                        case 'i':
                            if (mediaBuilder == null) {
                                ...
                                mediaBuilder.setInformation(Information.parse(matcher.group(2).trim()));
                            }
                        ...
                        case 'c':
                            if (mediaBuilder == null) {
                                sesionBuilder.setConnection(Connection.parse(matcher.group(2).trim()));
                            }
                            else {
                                mediaBuilder.setConnection(Connection.parse(matcher.group(2).trim()));
                            }
                        ...
                        case 'b':
                            if (mediaBuilder == null) {
                                sesionBuilder.setBandwidth(Bandwidth.parse(matcher.group(2).trim()));
                            }
                            else {
                                mediaBuilder.setBandwidth(Bandwidth.parse(matcher.group(2).trim()));
                            }

                            break;

                        case 't':
                            sesionBuilder.setTime(Time.parse(matcher.group(2).trim()));
                            break;

                        case 'k':
                            if (mediaBuilder == null) {
                                sesionBuilder.setKey(Key.parse(matcher.group(2).trim()));
                            }
                            else {
                                mediaBuilder.setKey(Key.parse(matcher.group(2).trim()));
                            }
                        ...
                        case 'a':
                            if (mediaBuilder == null) {
                                sesionBuilder.setAttribute(Attribute.parse(matcher.group(2).trim()));
                            }
                            else {
                                mediaBuilder.setAttribute(Attribute.parse(matcher.group(2).trim()));
                            }
                            break;
                        ...
                        case 'm':
                            ...
                            if (mediaBuilder != null) {
                                sesionBuilder.setMedia(mediaBuilder.build());
                            }

                            mediaBuilder = new MediaDescription.Builder();
                            mediaBuilder.setMedia(Media.parse(matcher.group(2).trim()));
                            break;
                    }
                }
            }
    ...
    public static class Builder {
        ...
        public Builder setVersion(ProtoVersion version) {
        ...
        public Builder setSessionName(SessionName name) {
        ...
        public Builder setMedia(MediaDescription media) {
