...
public final class CodecSpecificDataUtil {
   ...
   * Builds a H.264 configuration information, as defined in RFC 6814 and ISO/IEC 14496-10
   *
   * @param sps A byte array containing the H.264 Sequence Parameter Set
   * @param pps A byte array containing the H.264 Picture Parameter Set
   * @return The H.264 configuration information
   */
  public static List<byte[]> buildH264SpecificConfig(byte[] sps, byte[] pps)
      throws IllegalArgumentException {
    /* For H.264 MPEG4 Part15, the CodecPrivateData field must contain SPS and PPS in the following
      form, base16-encoded: [start code][SPS][start code][PPS], where [start code] is the following
      four bytes: 0x00, 0x00, 0x00, 0x01 */
    List<byte[]> codecSpecificData = new ArrayList<>();
    codecSpecificData.add(CodecSpecificDataUtil.buildNalUnit(sps, 0, sps.length));
    codecSpecificData.add(CodecSpecificDataUtil.buildNalUnit(pps, 0, pps.length));
    return codecSpecificData;
  }
